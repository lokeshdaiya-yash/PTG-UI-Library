  ...
  try {
    ...
    if(competency.length > 0 ){
      ...
      for(let i=0; i<competency.length; i++){
        if(competency[i]['name'].toLowerCase()===request.body.name.toLowerCase()){
          checking=true;
          break;
        }
      }
      if(checking==false){
        ...
        const newcompetency = new Competencies(competencies);
         await newcompetency.save();
        response.status(200).json({newcompetency, message: "competency added successfully"});
      }
      else{
         response.status(404).json({ message:"competency (" + request.body.name+") is already exist"  });
    }}
  } catch (error) {
    response.status(409).json({ message: error.message });
  }
