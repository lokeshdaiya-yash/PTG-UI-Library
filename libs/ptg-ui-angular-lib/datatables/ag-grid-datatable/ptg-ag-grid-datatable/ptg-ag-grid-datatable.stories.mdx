...
```
    import { PtgAgGridDatatableModule } from ' @ptg-ui/angular'
```    
...
9] enableCustomFilter - 

enableCustomFilter control is used to hide and show the customPaginationClass.  
  
enableCustomFilter control have two value :

a] True - customPagination is visible .

b] False - customPagination is hide .


<Canvas><Story id="component-ptgaggriddatatablecomponent--enable-custom-filter" /></Canvas>



10] customPaginationClass -

  We can set the customPaginationClass for the pagination.

  When we open DATATABLE_WITH_ACCORDIAN we can see the  customPaginationClass class.
