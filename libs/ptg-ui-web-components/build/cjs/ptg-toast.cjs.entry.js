...
const PtgToast = class {
  ...
  render() {
    return (index.h("div", null, index.h("button", { class: "btn btn-primary", onClick: () => {
        this.divElement.setAttribute('id', 'open');
        setTimeout(() => {
          this.divElement.removeAttribute('id');
        }, this.delay);
      } }, "Click Here"), index.h("div", { class: `toast ${this.appearance}`, id: this.id1, ref: (el) => this.divElement = el }, index.h("strong", null, this.type), index.h("p", null, this.discription))));
  }
};
PtgToast.style = ptgToastCss;

exports.ptg_toast = PtgToast;
