...
const PtgCalender = class {
  ...
  render() {
    const date = this.getValidDate();
    return (h("div", { class: "calendar material" }, h("header", null, h("span", { onClick: this.switchToPreviousMonth }, '<'), h("span", null, this.monthNames[date.month - 1], " ", date.year), h("span", { onClick: this.switchToNextMonth }, '>')), h("div", { class: "day-names" }, this.dayNames.map((dayName) => (h("span", null, dayName)))), h("div", { class: "days-in-month" }, this.daysInMonth.map((day, index) => {
      ...
      if (index < this.fillStartCount || index >= this.fillEndCount) {
        return (h("span", { class: "disabled" }, this.showFillDays ? day : ''));
      }
      else {
        return (h("span", { onClick: () => this.daySelectedHandler(day) }, h("i", { class: classNameDigit }, day)));
      }
    }))));
  }
...
export { PtgCalender as ptg_calender };
